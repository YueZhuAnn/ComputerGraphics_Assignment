...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Create Button, and reset the triangle if it was clicked:
		...
		// Retrieve green color component from slider and store in the first element of
		// m_shape_color.
		ImGui::SliderFloat("Green Channel", &m_shape_color.g, 0.0f, 1.0f);

		// Retrieve blue color component from slider and store in the first element of
		// m_shape_color.
		ImGui::SliderFloat("Blue Channel", &m_shape_color.b, 0.0f, 1.0f);

		// Retrieve rotation angle from slider and store in the first element of
		// m_shape_rotation.
		ImGui::SliderFloat("Rotate Channel", &m_shape_rotation, 0.0f, 2*PI);
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			m_shape_size = m_shape_size + 0.1 > 10 ? 10 : m_shape_size + 0.1;
		...
		if (key == GLFW_KEY_MINUS) {
			m_shape_size = m_shape_size - 0.1 < 0 ? 0 : m_shape_size - 0.1;
