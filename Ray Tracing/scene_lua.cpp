...
#  define GRLUA_DEBUG(x) do { cerr << x << endl; } while (0)
#  define GRLUA_DEBUG_CALL do { cerr << __FUNCTION__ << endl; } while (0)
#else
...
// The "userdata" type for a bumpmap. Objects of this type will be
// allocated by Lua to represent bumpmap.
...
// The "userdata" type for a Focal. Objects of this type will be
// allocated by Lua to represent focal len.
struct gr_focal_ud {
  Focal* focal;
};
...
int gr_nh_torus_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new Torus(pos, R, r));
...
int gr_light_cmd(lua_State* L)
{
  ...
  l.colour = vec3(col[0], col[1], col[2]);
  
  l.energy = luaL_checknumber(L, 4);
  l.brightness = luaL_checknumber(L, 5);
...
// Set a focal len
extern "C"
int gr_focal_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;

  gr_focal_ud* data = (gr_focal_ud*)lua_newuserdata(L, sizeof(gr_focal_ud));
  data->focal = 0;
  ...
  data->focal = new Focal(p,f,n);

  luaL_newmetatable(L, "gr.focal");
  lua_setmetatable(L, -2);
...
int gr_render_cmd(lua_State* L)
{
  ...
  vec3 ambient(ambient_data[0], ambient_data[1], ambient_data[2]);
  ...
  gr_focal_ud* matdata = (gr_focal_ud*)luaL_checkudata(L, -1, "gr.focal");
  luaL_argcheck(L, matdata != 0, 11, "Focal Len");
  Focal* focal = matdata-> focal;
  
	Image im( width, height);
	Project_Render(root->node, im, eye, view, up, fov, ambient, lights, focal, materialmap);
  im.savePng( filename );
...
int gr_material_cmd(lua_State* L)
{
  ...
  data->material = new PhongMaterial(vec3(kd[0], kd[1], kd[2]),
                                     vec3(ks[0], ks[1], ks[2]),
                                     shininess);

  materialmap->addMaterial(data->material);
...
int gr_texture_cmd(lua_State* L)
{
  ...
  const char* texture_fname = luaL_checkstring(L, 1);
	string sfname( texture_fname );

  vec3 ks;
  get_tuple(L, 2, &ks[0], 3);

  double shininess = luaL_checknumber(L, 3);
  
  data->texture = new Texture(sfname, ks, shininess);
  materialmap->addTexture(data->texture);
...
int gr_bumpmap_cmd(lua_State* L)
{
  ...
  const char* bump_fname = luaL_checkstring(L, 1);
	string sfname( bump_fname );
  
  data->bumpmap = new Bumpmap(sfname);
  materialmap->addBumpmap(data->bumpmap);
...
int gr_node_add_child_cmd(lua_State* L)
{
  ...
  if(child->m_nodeType == NodeType::GeometryNode)
  {
    if(!lua_isnil(L, 3))
    {
      gr_material_ud* matdata = (gr_material_ud*)luaL_checkudata(L, 3, "gr.material");
      luaL_argcheck(L, matdata != 0, 3, "Material expected");
      Material* material = matdata->material;
      mid = materialmap->findMaterialID(material);
    }
    if(!lua_isnil(L, 4))
    {
      gr_texture_ud* matdata = (gr_texture_ud*)luaL_checkudata(L, 4, "gr.texture");
      luaL_argcheck(L, matdata != 0, 4, "Texture expected");
      Texture* texture = matdata->texture;
      tid = materialmap->findTextureID(texture);
    }
    if(!lua_isnil(L, 5))
    {
      gr_bumpmap_ud* matdata = (gr_bumpmap_ud*)luaL_checkudata(L, 5, "gr.bumpmap");
      luaL_argcheck(L, matdata != 0, 5, "Bumpmap expected");
      Bumpmap* bumpmap = matdata->bumpmap;
      bid = materialmap->findBumpID(bumpmap);
    }
  }
  self->add_child(child, mid, tid, bid);
  return 0;
}


// Set a node's Reflection
extern "C"
int gr_node_set_reflection_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  self->setReflection(factor);

  return 0;
}

// Set a node's Refraction
extern "C"
int gr_node_set_refraction_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  factor =  luaL_checknumber(L, 2);
  transparency = luaL_checknumber(L, 3);
  self->setRefraction(factor, transparency);
...
int gr_node_set_texture_cmd(lua_State* L)
{
  ...
  Texture* texture = matdata->texture;
  unsigned id = materialmap->findTextureID(texture);
  self->setTexture(id);
	return 1;
}

// Set a node's Bump mapping
extern "C"
int gr_node_set_bumpmap_cmd(lua_State* L)
{
  ...
  gr_bumpmap_ud* matdata = (gr_bumpmap_ud*)luaL_checkudata(L, 2, "gr.bumpmap");
  luaL_argcheck(L, matdata != 0, 2, "Bumpmap expected");

  Bumpmap* bumpmap = matdata->bumpmap;
  unsigned id = materialmap->findBumpID(bumpmap);
  self->setBumpMap(id);
...
int gr_node_scale_cmd(lua_State* L)
{
  ...
  self->scale(vec3(values[0], values[1], values[2]));
...
int gr_node_translate_cmd(lua_State* L)
{
  ...
  self->translate(vec3(values[0], values[1], values[2]));
...
int gr_node_rotate_cmd(lua_State* L)
{
  ...
  luaL_argcheck(L, axis_string
                && strlen(axis_string) == 1, 2, "Single character expected");
  char axis = tolower(axis_string[0]);
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_reflection", gr_node_set_reflection_cmd},
  {"set_refraction", gr_node_set_refraction_cmd},
...
bool run_lua(const string& filename)
{
  ...
  if (luaL_loadfile(L, filename.c_str()) || lua_pcall(L, 0, 0, 0)) {
    cerr << "Error loading " << filename << ": " << lua_tostring(L, -1) << endl;
    return false;
  }
